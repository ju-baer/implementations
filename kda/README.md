...
notebook skips that — it doesn't change what the layer *computes*, only how
...
weighted mixture of all previous outputs. That mixture — not just the raw
previous layer's output — is what gets fed into the current layer.
...
there's no need to partition — it uses the Full form directly, which is
...
## 8. Reference
Kimi Team, [*Kimi K3: Open Frontier Intelligence*](https://arxiv.org/pdf/2607.24653), 2026. Section and equation numbers referenced throughout the code comments correspond to this report, read the code and the report side by side.
